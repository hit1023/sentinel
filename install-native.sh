#!/usr/bin/env bash
# hit-linux-ids (SENTINEL) エージェント ネイティブインストーラー（Linux/systemd）
#
# Dockerを使わず、GitHub Releasesの単一バイナリをダウンロードして
# systemdサービスとして常駐させる。curl一発でも実行できる:
#
#   curl -fsSL https://raw.githubusercontent.com/hit1023/sentinel/main/install-native.sh | sudo bash -s -- \
#     --webui-url http://192.168.0.18:8877 --token xxxxxxxx --host-label server-01
#
# 使い方:
#   ./install-native.sh                対話形式でセットアップ
#   ./install-native.sh --non-interactive \
#       --webui-url http://192.168.0.18:8877 \
#       --token xxxxxxxx \
#       --host-label server-01 \
...
SYSTEMD_UNIT="/etc/systemd/system/sentinel-agent.service"
...
if [ "$(id -u)" -ne 0 ]; then
  ...
  echo "        例: sudo ./install-native.sh" >&2
  exit 1
fi

echo "=================================================="
echo " hit-linux-ids (SENTINEL) ネイティブインストーラー"
echo "=================================================="
echo
...
case "$ARCH" in
  ...
  *)
    echo "エラー: 未対応のアーキテクチャです: $ARCH（linux-x86_64のみ対応）" >&2
    exit 1
    ;;
esac
ASSET_NAME="sentinel-agent-linux-${ASSET_ARCH}"
...
cp "$TMP_DIR/sentinel-agent.service" "$SYSTEMD_UNIT"
...
echo "   ログ確認: journalctl -u sentinel-agent -f"
